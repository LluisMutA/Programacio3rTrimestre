...
public class Containers {
    ...
    static void llistes1() {
        List<Integer> enters = new ArrayList<>();// Crea llista d'enters anomenada "l1"

        enters.add(10);
        enters.add(20);
        enters.add(15);// Afegeix els números 10, 20 i 15

        System.out.println(enters); // Imprimeix la llista

        enters.remove(1);// Elimina el segon element

        System.out.println(enters);// Imprimeix la llista


        List<Integer> enters2 = new ArrayList<>();// Crea llista d'enters anomenada "l2"

        enters2.add(3);
        enters2.add(6);
        enters2.add(9);// Afegeix els números 3, 6 i 9

        for (Integer i : enters) { // Afegeix la llista l1 a la llista l2
            enters2.add(i);
        }

        System.out.println(enters2);// Imprimeix la llista l2


        enters.clear();// Elimina tots els valors de l1

        int contador = 0;// Afegeix els tres primers elements de l2 a l1
        for (Integer i : enters2) {
            enters.add(i);
            contador++;
            if (contador == 3) break;
        }
        //System.out.println(enters);
        for (Integer i : enters){// Comprova si dins l1 hi ha el número 6
            if(enters(i) == 6){
